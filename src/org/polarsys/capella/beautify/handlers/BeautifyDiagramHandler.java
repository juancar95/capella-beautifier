...
public class BeautifyDiagramHandler extends AbstractHandler {
    ...
    private void normalizeZeroSizedNodes(DiagramEditPart editPart) {
        ...
        while (!queue.isEmpty()) {
            ...
            try {
                int w = ShapeUtil.getNodeWidth(node), h = ShapeUtil.getNodeHeight(node);
                boolean changed = false;
                if (w <= 1) { ShapeUtil.setNodeWidth(node, 80); changed = true; }
                if (h <= 1) { ShapeUtil.setNodeHeight(node, 32); changed = true; }
